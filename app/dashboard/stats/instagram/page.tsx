  ...
  const loadAccountStats = async () => {
    try {
      ...
      const response = await fetch(
        `/api/instagram/analytics?timeframe=${selectedTimeframe}`
      );
      if (response.ok) {
      ...
      <Card className="bg-horror-surface border-horror-border">
        <CardContent className="text-center py-12">
          ...
          <p className="text-gray-400">
            Keine Instagram-Accounts für Statistiken verfügbar
          </p>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-xs text-green-400">
                          +
                          {
                            currentAccountStats.metrics.followers_growth
                              .percentage
                          }
                          %
                        </span>
                    ...
                    <div>
                      ...
                      <p
                        className={`text-2xl font-bold ${getEngagementColor(
                          currentAccountStats.metrics.engagement_rate.current
                        )}`}
                      >
                        {currentAccountStats.metrics.engagement_rate.current.toFixed(
                          1
                        )}
                        %
                      </p>
                      <p className="text-xs text-gray-400 mt-1">
                        Ø{" "}
                        {currentAccountStats.metrics.engagement_rate.average.toFixed(
                          1
                        )}
                        %
                      </p>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        Reichweite:{" "}
                        {currentAccountStats.metrics.impressions.reach.toLocaleString()}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-xs text-horror-accent">
                          CTR:{" "}
                          {currentAccountStats.metrics.click_through_rate.toFixed(
                            1
                          )}
                          %
                        </span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card className="bg-horror-surface border-horror-border">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-xl font-bold text-white">
                    {currentAccountStats.metrics.saves}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="bg-horror-surface border-horror-border">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-xl font-bold text-white">
                    {currentAccountStats.metrics.shares}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="bg-horror-surface border-horror-border">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-xl font-bold text-white">
                    {currentAccountStats.metrics.story_views}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="bg-horror-surface border-horror-border">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-xl font-bold text-white">
                    {currentAccountStats.metrics.website_clicks}
                  </p>
                </CardContent>
              </Card>
            </div>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {currentAccountStats.metrics.top_posts.map((post, index) => (
                    <div
                      key={post.id}
                      className="bg-horror-bg/50 rounded-lg p-4"
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span
                            className={getEngagementColor(post.engagement_rate)}
                          >
            ...
            <Card className="bg-horror-surface border-horror-border">
              ...
              <CardContent>
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400">
                    Detaillierte Engagement-Analyse wird implementiert
                  </p>
                  <p className="text-sm text-gray-500 mt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="bg-horror-surface border-horror-border">
                ...
                <CardContent className="space-y-4">
                  {currentAccountStats.metrics.audience_demographics.age_groups.map(
                    (group) => (
              ...
              <Card className="bg-horror-surface border-horror-border">
                ...
                <CardContent className="space-y-4">
                  {currentAccountStats.metrics.audience_demographics.locations.map(
                    (location) => (
            ...
            <Card className="bg-horror-surface border-horror-border">
              ...
              <CardContent>
                <div className="space-y-3">
                  {currentAccountStats.metrics.hashtag_performance.map(
                    (hashtag) => (
                      <div
                        key={hashtag.hashtag}
                        className="flex items-center justify-between p-3 bg-horror-bg/50 rounded-lg"
                      >
          ...
          <TabsContent value="timing" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-white">
                        {currentAccountStats.metrics.posting_frequency.current}{" "}
                        Posts/Woche
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-400">Optimal:</span>
                      <span className="text-horror-accent">
                        {currentAccountStats.metrics.posting_frequency.optimal}{" "}
                        Posts/Woche
                      </span>
                    </div>
                    <Progress
                      value={
                        (currentAccountStats.metrics.posting_frequency.current /
                          currentAccountStats.metrics.posting_frequency
                            .optimal) *
                        100
                      }
                      className="h-2"
              ...
              <Card className="bg-horror-surface border-horror-border">
                ...
                <CardContent>
                  <div className="space-y-2">
                    {currentAccountStats.metrics.best_posting_times.map(
                      (time, index) => (
                        <Badge
                          key={index}
                          variant="secondary"
                          className="bg-horror-accent/20 text-horror-accent mr-2 mb-2"
                        >
                          {time}
                        </Badge>
                      )
                  ...
                  <p className="text-sm text-gray-400 mt-4">
                    Basierend auf höchstem Engagement der letzten{" "}
                    {selectedTimeframe}
                  </p>
                </CardContent>
              </Card>
